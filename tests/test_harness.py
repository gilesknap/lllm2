...
class HarnessTests(unittest.TestCase):
    def test_running_engine_metadata(self):
        with patch.object(harness.Engine, 'request', side_effect=[
    ...
    def test_session_configuration_and_cleanup(self):
        ...
        def call(argv, env):
            ...
            if name == 'claude':
                ...
                provider = json.loads(source.split('"lllm2", ', 1)[1].removesuffix('); }\n'))
        ...
        with patch.dict(os.environ, {'ANTHROPIC_API_KEY': 'existing', 'PRESERVED': 'yes'}), \
                patch.object(harness.shutil, 'which', side_effect=lambda name: '/bin/' + name), \
                patch.object(harness, 'served_model', return_value=(
                    'http://127.0.0.1:1920', model, 16384, 4)), \
                patch.object(harness.subprocess, 'call', side_effect=call):
            for name in ('claude', 'codex', 'pi'):
                self.assertEqual(harness.run_harness(name, ['-p', 'prompt with spaces']), 17)
    ...
    def test_missing_cli_and_signal_exit(self):
        ...
        with patch.object(harness.shutil, 'which', return_value='/bin/pi'), \
                patch.object(harness, 'served_model', return_value=('http://localhost', 'm', 4096, 1)), \
