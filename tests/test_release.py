...
class ReleaseTagTests(unittest.TestCase):
    def check_distribution(self, ref_type, ref_name):
        with patch.dict(os.environ, GITHUB_REF_TYPE=ref_type, GITHUB_REF_NAME=ref_name), \
                patch('subprocess.run') as cli, contextlib.redirect_stdout(io.StringIO()):
    ...
    def test_wrong_or_malformed_tag_is_rejected(self):
        for tag in ('9.9.999', 'v9.9.999', 'release-' + version('lllm2'),
                    'vv' + version('lllm2')):
            with self.subTest(tag=tag), self.assertRaisesRegex(AssertionError, 'does not match'):
