...
from lllm2.gpu_tables import table_hardware
...
ENGINE = {
    ...
    "devices": ["CUDA0"],
    "error": None,
    "cuda_graph": {"supported": True, "reason": "Probed.", "library": None},
    "cache_kernel": {"reason": "Probed.", "library": None},
    "environment": {},
}
...
class UncataloguedPlanTests(unittest.TestCase):
    ...
    def resolve(self, model, meta, host):
        selection = Settings.parse(
            ...
            else {"model": model, "engine": ENGINE["path"], "backend": "CUDA"}
        )
        with (
    ...
    def test_owner_q8_checkpoint_plans_the_full_context(self):
        ...
        s, result = self.resolve(Q8_PATH, Q8_META, host)
    ...
    def test_the_catalogued_quantisation_still_plans_the_same(self):
        ...
        s, _ = self.resolve(path, meta, table_hardware("modal", "RTX-PRO-6000"))
        self.assertEqual((s.context, s.slots), (1048576, 4))
    ...
    def test_the_catalogue_keeps_the_last_word_on_a_checkpoint_it_lists(self):
        ...
        meta["full_attention_layers"] = None
        catalogued, _ = self.resolve(path, dict(meta, kv_kib_per_token=None), host)
        doubled, _ = self.resolve(path, dict(meta, kv_kib_per_token=48.0), host)
    ...
    def test_an_uncalibrated_context_still_fits_the_card(self):
        ...
        with patch.dict(ENGINE, engine, clear=True):
            s, result = self.resolve(
                "/models/Homemade/Homemade-Q4_K_M.gguf", meta, host
            )
    ...
    def test_unplannable_checkpoint_is_clamped_and_says_so(self):
        ...
        s, result = self.resolve("/models/Mystery/Mystery-Q6_K.gguf", blind, host)
        notes = " ".join(result["notes"])
    ...
    def test_no_gpu_is_an_error_not_a_default(self):
        for host in (
            ...
            {"source": "local", "gpus": [], "error": None},
        ):
            with self.subTest(source=host["source"]):
