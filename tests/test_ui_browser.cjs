...
(async()=>{try{
 ...
 assert.equal(await run("document.querySelectorAll('#workloads input:checked').length"),0);
 assert.match(await run("$('benchmark-time').textContent"),/Context discovery only/);
 await run("document.querySelector('#workloads input[value=long-code]').click();$('sweep_prompts').click();$('search_context').click()");
 ...
 assert.match(await run("$('benchmark-time').textContent"),/no estimate/);
 await run("document.querySelectorAll('#workloads input:checked').forEach(x=>x.click());$('search_context').click()");
