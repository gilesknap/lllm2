...
META = {"error": None, "context": 262144, "template": "", "mtp": True}
...
class GpuOffloadTests(unittest.TestCase):
    def test_settings_accept_auto_and_preserve_manual_counts(self):
        ...
        for value in (-1, 1000, True, 1.5, "12"):
            with (
                ...
                self.assertRaisesRegex(ValueError, "GPU layers"),
            ):
    ...
    def test_auto_fits_placement_without_changing_context(self):
        with (
            patch("lllm2.settings.probe", return_value=PROBE),
            ...
            for layers in (None, 0, 12, 999):
                with self.subTest(layers=layers):
                    ...
                    if layers is None:
                        ...
                        self.assertEqual(
                            args[args.index("--gpu-layers") + 1], str(layers)
                        )

    def test_old_engine_requires_manual_layers(self):
        with (
            patch("lllm2.settings.probe", return_value={**PROBE, "flags": FLAGS[:-2]}),
            patch("lllm2.settings.metadata", return_value=META),
            patch("lllm2.settings.capabilities", return_value={}),
        ):
            ...
            self.assertIn(
                "--gpu-layers",
                launch_args(Settings(model="/model.gguf", gpu_layers=5), 1920),
            )

    def test_auto_clears_inherited_layer_override_only_in_child(self):
        parent = {"LLAMA_ARG_N_GPU_LAYERS": "999", "PRESERVE": "yes"}
        with patch(
            "lllm2.settings.engine_environment", side_effect=lambda _: dict(parent)
        ):
    ...
    def inherited(self, total_mib, fit=True):
        ...
        caps = {
            name: {"status": "available"} for name in ("flash", "cache", "draft-mtp")
        }
        with (
            patch.object(
                defaults,
                "probe",
                return_value=PROBE if fit else {**PROBE, "flags": FLAGS[:-2]},
            ),
            ...
            patch.object(
                defaults,
                "hardware",
                return_value={"gpus": [{"total_mib": total_mib, "uuid": "gpu"}]},
            ),
            patch.object(defaults, "command", return_value=(0, "512")),
        ):
            return defaults.inherited_defaults(
                Settings(model="/model.gguf", engine="/llama-server")
            )
    ...
    def test_8gb_gets_partial_offload_starting_point(self):
        ...
        self.assertIsNone(settings.gpu_layers)
        self.assertEqual(
            (settings.context, settings.slots, settings.speculation), (32768, 1, "none")
        )
    ...
    def test_24gb_retains_inherited_context_plan(self):
        ...
        self.assertNotIn(
            "full-GPU context estimate does not fit", " ".join(result["notes"])
        )
    ...
    def test_measured_profile_retains_its_tested_placement(self):
        measured = {"settings": Settings(gpu_layers=999).dict(), "source": "measured"}
        with (
            patch.object(defaults, "measured_defaults", return_value=(measured, [])),
...
class A400CatalogueDefaultsTests(unittest.TestCase):
    ...
    def resolve(
        ...
        *,
        ...
        selection = Settings(
            model="/models/" + entry["name"] + "/" + entry["file"],
            engine="/llama-server",
            context=65536,
            slots=4,
            gpu_layers=999,
        ...
        caps = {
            name: {
                "status": "available" if cache else "unsupported",
                "reason": "test engine",
            }
        ...
        host = {
            "gpus": [
                {"name": "NVIDIA RTX A400", "total_mib": total_mib, "uuid": "gpu"}
            ],
            "ram_gib": 32,
            "ram": {"available_gib": available_gib},
        }
        with (
            patch.object(defaults, "probe", return_value=p),
        ...
        with (
            patch("lllm2.settings.probe", return_value=p),
