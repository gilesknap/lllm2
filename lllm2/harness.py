...
def served_model():
    ...
    try:
        ...
        raise RuntimeError('The engine did not report a model, context window and slot count.') from error
...
def codex_args(base, model, window):
    ...
    return [arg for key, value in settings.items() for arg in ('-c', f'{key}={json.dumps(value)}')]
...
def pi_extension(base, model, window):
    provider = {
        ...
        'models': [{
            ...
            'contextWindow': window, 'maxTokens': min(32768, max(1, window // 4)),
            'compat': {'supportsStore': False, 'supportsDeveloperRole': False,
                       'supportsReasoningEffort': False, 'maxTokensField': 'max_tokens'},
        }],
    }
    return 'export default function (pi) { pi.registerProvider("lllm2", ' + json.dumps(provider) + '); }\n'
...
def run_harness(name: str, args: list[str]) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix='lllm2-harness-') as directory:
        ...
        if name == 'claude':
            ...
            options = ['--extension', str(extension), '--provider', 'lllm2', '--model', model]
        else:
