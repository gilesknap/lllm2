...
def _library_digest(path, device, inode, size, mtime_ns, ctime_ns):
    ...
    if (stat.st_dev, stat.st_ino, stat.st_size, stat.st_mtime_ns, stat.st_ctime_ns) != (
        ...
        raise ValueError(
            "CUDA library changed during fingerprinting; retry inspection."
        )
    return digest
...
def cache_kernel_support(binary, backend):
    ...
    try:
        ...
        out["library"] = {"path": str(library), "size": stat.st_size, "sha256": digest}
        if digest == "b81f5da083d4c25345569268819bce5f7e14b45de19d055f634db3e22bbcef61":
            out.update(
                mixed_gpu_kernel="unsupported",
...
def cuda_graph_support(binary):
    ...
    try:
        ...
        return {
            "supported": supported,
            "library": info,
            "reason": "Compiled CUDA streams switch found."
            if supported
            else "Adjacent CUDA library does not contain the streams switch.",
        }
    except OSError:
        return {
            "supported": False,
            "library": None,
            "reason": "Cannot confirm streams support in an adjacent readable CUDA library.",
        }
...
def host_memory():
    ...
    try:
        ...
        return {
            "total_gib": round(total / 2**20, 1),
            "used_gib": round((total - available) / 2**20, 1),
            "available_gib": round(available / 2**20, 1),
        }
    except (OSError, ValueError, KeyError, IndexError):
        return {"total_gib": None, "used_gib": None, "available_gib": None}
            ...
            found.append(
                dict(
                    **info,
                    ...
                    metadata={k: v for k, v in m.items() if k != "template"},
                )
            )
    return found
...
def _probe(path, size, mtime_ns):
    ...
    _, devices = (
        command([path, "--list-devices"], env=env)
    ...
    return {
        "path": path,
        "version": version.strip()[:1000],
        "flags": flags,
        "help": help_text,
        "devices": available,
        "device_output": devices[:2000],
        "error": None if rc == 0 else help_text[:500],
        "sha256": digest,
    }
