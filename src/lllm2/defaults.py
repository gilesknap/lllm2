...
Where nothing can be computed the settings say so in a note rather than keeping
a silent 4096.
"""
...
def planner_evidence(entry, meta):
...
def inherited_defaults(selection):
    ...
    if not cards:
        ...
        raise ValueError(
            f"No {s.backend} GPU is available for this launch. "
    ...
    plan, unknown = planner_evidence(entry, m)
    ...
    if uncalibrated:
        ...
        s.context = uncalibrated_context(ceiling, plan, cards, s.cache, s.backend)
        s.slots = 1
        notes.append(
...
def uncalibrated_context(ceiling, plan, cards, cache, backend, desktop=True):
