...
function benchmarkCost(){
 ...
 $('benchmark-cost').textContent=`Per configuration: ${search?'context discovery first (up to 12 quick load checks and up to 8 context probes), then ':''}${sizes.length*workloads*repeats} speed samples (${sizes.map(n=>n.toLocaleString()).join(' / ')} input tokens).`;
}
