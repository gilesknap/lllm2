...
EXPECTED = {
    key: SOURCE
    if key == "source-copy"
    else SOURCE.replace("if amount < 0:", "if amount <= 0:")
    for key in TASKS
}
...
EXPECTED["context-retrieval-edit"] = (
    CONTEXT_SOURCE.replace("retry_limit = 0", "retry_limit = 7")
    .replace("item_limit = 0", "item_limit = 143")
    .replace("deadline_seconds = 0", "deadline_seconds = 29")
    .replace("items < item_limit", "items <= item_limit")
)
...
def source_prompt(bench, settings, workload, tokens, timeout):
    ...
    text = (
        "Unrelated reference context, not part of the requested output:\n"
        + marker
        + "\n\n"
        + task
    )
    ...
    def tokenize(content, special=False):
        return bench.req(
            "/tokenize",
            {"content": content, "add_special": special, "parse_special": True},
            timeout,
        )["tokens"]
    ...
    if room < 0:
        raise ValueError(
            f"Source workload requires at least {len(head) + len(tail)} input tokens to preserve the complete excerpt and instructions."
        )
    ...
    return actual, {
        "generator": "source-snapshot-v1",
        "workload": workload,
        "source": SOURCE,
        "source_sha256": hashlib.sha256(SOURCE.encode()).hexdigest(),
        "instruction": TASKS[workload],
        "reference_sha256": hashlib.sha256(REFERENCE.encode()).hexdigest(),
        "padding_tokens": room,
        "token_ids": actual,
        "expected_output": EXPECTED[workload],
...
def context_prompt(bench, settings, tokens, timeout):
    ...
    if room < 0:
        raise ValueError(
            f"Context retrieval/edit requires at least {tokens - room} input tokens to preserve all facts and instructions."
        )
    reference = tokenize(REFERENCE)
    if not reference:
        raise RuntimeError("Could not tokenize reference context.")
    ...
    for index, fact in enumerate(facts):
        locations.append(
            {
                "name": CONTEXT_FACTS[index][0],
                "start_token": len(actual),
                "end_token": len(actual) + len(fact),
            }
        )
        actual.extend(fact)
        if index == 0:
    ...
    actual.extend(tail)
    return actual, {
        "generator": "dispersed-retrieval-edit-v2",
        "workload": workload,
        "source": CONTEXT_SOURCE,
        "source_sha256": hashlib.sha256(CONTEXT_SOURCE.encode()).hexdigest(),
        "instruction": TASKS[workload],
        "facts": dict(CONTEXT_FACTS),
        "fact_token_offsets": locations,
        "reference_sha256": hashlib.sha256(REFERENCE.encode()).hexdigest(),
        "padding_tokens": room,
        "token_ids": actual,
        "expected_output": EXPECTED[workload],
...
def adherence(response, expected):
    ...
    limited = bool(
        ...
        or response.get("stopped_limit")
        or response.get("stop_type") == "limit"
    )
    matches = payload == expected.strip()
    return {
        "status": "passed" if matches and not limited else "failed",
        "exact_payload_match": matches,
        "output_limit_reached": limited,
        "thinking_characters": len(thinking) if thinking is not None else None,
