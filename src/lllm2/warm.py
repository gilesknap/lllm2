...
def measure_turn(bench, s, opts, sample, prompt, cache_prompt):
    ...
    progress = {
        "output": "",
        "output_token_ids": [],
        "first_token_event_seconds": None,
    ...
    def observe(event):
        ...
        with lock:
            ...
            if "prompt_progress" not in event and event.get("stop") is not True:
                ...
                if (
                    len(progress["output_token_ids"]) > opts["output_tokens"] + 256
                    or len(progress["output"]) > 2 * 1024 * 1024
                ):
                    raise RuntimeError(
                        "Generated stream exceeded bounded output budget."
                    )
    ...
    sampler.start()
    try:
        response = engine.stream_completion(
            {
                "prompt": prompt,
                "n_predict": opts["output_tokens"],
                "temperature": 0,
                "seed": 42,
        ...
        sample["response"] = {
            ...
            if k not in ["prompt", "content", "tokens"]
        }
        ...
        sample.update(
            ...
            execution_settings=execution_settings(
                s, engine.execution_environment, engine.state()["logs"], response
            ),
        )
        if any(
            type(n) is not int or n < 0 for n in [processed, reused]
        ) or processed + reused != len(prompt):
            raise RuntimeError(
                "Missing or inconsistent processed/reused prompt accounting; cannot establish reuse."
            )
        if (
            response.get("tokens_evaluated") != len(prompt)
            or response.get("truncated")
            or predicted != opts["output_tokens"]
        ):
            raise RuntimeError("Incomplete warm workload or context truncation.")
        with lock:
            if (
                len(progress["output_token_ids"]) != predicted
                or progress["first_token_event_seconds"] is None
            ):
                raise RuntimeError(
                    "Stream token IDs do not match the completed output budget (this engine may omit byte tokens while buffering UTF-8); exact history cannot be reconstructed."
                )
        ...
        sample["peak_engine_rss_mib"] = max(
...
def run_conversation(bench, s, opts, result):
    ...
    for repetition in range(1, opts["repeats"] + 1):
        ...
        def run(
            ...
            *,
            ...
            if len(prompt) + opts["output_tokens"] + 32 > s.context:
                raise ValueError(
                    "Conversation turn exceeds its bounded context budget."
                )
            sample = {
                "workload": "warm-conversation",
                "turn": turn,
                "control": control,
                "repetition": repetition,
                "status": "running",
                "input_tokens": len(prompt),
                "output_budget": opts["output_tokens"],
                "slots": 1,
            ...
            save()
            bench.update(
                phase=f"Warm conversation {repetition}/{opts['repeats']}: {'uncached replay ' if control else ''}{turn}"
            )
            try:
                ...
                sample["engine_log_tail"] = (
                    logs[starts[-1] + 1 :][-80:] if starts else []
                )
                save()
            return sample
        ...
        away = list(base)
        away[8:-64] = (replacement * (len(base) // len(replacement) + 1))[
            : len(base) - 72
        ]
        ...
        result["reuse_summary"] = [
            {
                ...
                "turn": x["turn"],
                "control": x["control"],
                "processed_tokens": x.get("processed_tokens"),
                "reused_tokens": x.get("reused_tokens"),
                "status": x["status"],
            }
