...
def reuse(track: str, destination: Path, repository: str) -> bool:
    ...
    releases = [release for page in pages(result.stdout) for release in page]
    # Public assets take precedence over draft seeds, regardless of listing order.
    releases.sort(key=lambda release: bool(release.get("draft")))
    for release in releases:
        if release.get("prerelease"):
            continue
        names = {
            item["name"] for item in release["assets"] if item["state"] == "uploaded"
        }
        if not {asset, asset + ".sha256"}.issubset(names):
            continue
        if not release.get("draft"):
            print(
                f"Found {asset} on published release {release['tag_name']}; no build or upload"
            )
            return True
...
if __name__ == "__main__":
    ...
    if output := os.environ.get("GITHUB_OUTPUT"):
        with open(output, "a") as handle:
            ...
            published = (
                found and not (Path(sys.argv[2]) / asset_name(sys.argv[1])).exists()
            )
