...
| `lllm2 engines install cuda\|vulkan` | Build an engine; accepts `--ref`, `--name`, `--jobs`, `--cuda-architectures` and `--skip-checks`. Missing prerequisites are reported, with the commands that install them, before anything is downloaded. |
