...
| `lllm2 modal probe --gpu TYPE [--json]` | Run a short, billed probe container, print the GPU name, memory, engine devices and engine sha256, and save the result for later launches. |
...
| `lllm2 modal stop [CALL_ID\|--all] [--force]` | Stop one call, or every orphaned call with `--all`; `--force` also stops calls that a running session owns. |
